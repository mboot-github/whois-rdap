...
def mailcheck(data):
    return re.search(loose_emailaddr_exp, data)
    # Loose Email Check
...
def abusecheck(data):
    return re.search(abuse_exp, data)
...
class Cache:
    ...
    def Load(self):
        ...
        with open(self.backing_store, "r", newline="") as csvfile:
            csv.reader(csvfile) #  W0612 local variable 'reader' is assigned to but never used [pyflakes]
            entry = CacheEntry()
            entry.read(csvfile)
            self.lookup[key] = entry #  W0612 local variable 'reader' is assigned to but never used [pyflakes]
    ...
    def Change(self, key, value, expires=None):
        ...
        if entry is not None:
            entry.value = value
            entry.expires = datetime.now() + datetime.timedelta(seconds=expires)
    ...
    def Expire(self):
        ...
        for item in self.items:
            if item.expires > datetime.now():
                ...
                elligible.append(entry.key) # E0602 undefined name 'entry' [pyflakes]
...
def BuildParser():
    ...
    if __Parser__ is None:
        parser = __Parser__ = argparse.ArgumentParser(
            description="Whois Python Module and Utility"
        )

        parser.add_argument(
            "-d", "--debug", action="store_true", help="Enter debug mode"
        )
        parser.add_argument(
            "--noshow", action="store_true", help="Return output, but don't display it"
        )
        ...
        parser.add_argument(
            "-p",
            default=0,
            help="Add pause in seconds between RDAP calls",
            required=False,
        )
        parser.add_argument(
            "-f",
            "--file",
            "--whodey",
            help="File with list of IP's to lookup",
            required=False,
        )
        ...
        parser.add_argument(
            "-e",
            "--expires",
            help="Time in seconds for cache expiration",
            required=False,
        )
...
Initialize()  # allways run initialize on import
#
# Main Loop
#
